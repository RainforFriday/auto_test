...
class ApcReg():
    def __init__(self, UART2, REG_LOW = "40348060", REG_HIGH = "40348064", is22=0):
        # super(self).__init__()
        self.UART2 = UART2
    ...
    def swMode(self):
        if self.is22:
            self.UART2.sendcmd("w 403441D0 e")
            time.sleep(0.02)
            self.UART2.sendcmd("w 403441B8 222")
            self.UART2.sendcmd("w 403441B8 0")
            self.UART2.sendcmd("w 403441D0 9")
            self.UART2.sendcmd("w 403441B8 444")
            self.UART2.sendcmd("w 403441B8 555")
        else:
    ...
    def hwMode(self):
        if self.is22:
            self.UART2.sendcmd("w 403441B8 111")
            self.UART2.sendcmd("w 403441B8 0")
            self.UART2.sendcmd("w 403441B8 222")
            self.UART2.sendcmd("w 403441B8 333")
            self.UART2.sendcmd("w 403441D0 e")
            self.UART2.sendcmd("w 403441D0 6")
            self.UART2.sendcmd("w 403441B8 1333")
        else:
    ...
    def release_apc_table_gain(self):
        self.REG_LOW = '403440a0'
        self.write_reg_pos(14, 14, 0b0)    # tx_gain_dr
        self.write_reg_pos(15, 15, 0b1)   # tx_gain_dr_pulse

    def fix_8822_apc_ant0_tx_gain_reg(self, val):
        self.REG_LOW = '403441a4'
        self.write_reg_pos(6, 6, 0b1)    # tx_gain_dr
        self.write_reg_pos(5, 0, val)   # 0xac
        self.write_reg_pos(7, 7, 0b1)   # tx_gain_dr_pulse

    def release_8822_apc_ant0_tx_gain_reg(self):
        self.REG_LOW = '403441a4'
        self.write_reg_pos(6, 6, 0b0)    # tx_gain_dr
        self.write_reg_pos(7, 7, 0b1)   # tx_gain_dr_pulse

    def set_8822_wapc_common_bit(self, addrs, high_bit, low_bit, val):
        if self.is22:
            if high_bit < 64:
                self.REG_LOW = addrs[0]
                self.REG_HIGH = addrs[1]
                self.wapc(high_bit, low_bit, val)
            else:
                self.REG_LOW = addrs[1]
                self.REG_HIGH = addrs[2]
                self.wapc(high_bit-32, low_bit-32, val)

    def get_8822_rapc_common_bit(self, addrs, high_bit, low_bit):
        if self.is22:
            if high_bit < 64:
                self.REG_LOW = addrs[0]
                self.REG_HIGH = addrs[1]
                res = self.rapc(high_bit, low_bit)
            else:
                self.REG_LOW = addrs[1]
                self.REG_HIGH = addrs[2]
                res = self.rapc(high_bit-32, low_bit-32)
            return int(res, 2)

    def set_8822_apc_wf_hb0_pa_vm_ref_bit(self, addrs, val):
        if self.is22:
            self.REG_HIGH = addrs[1]
            self.wapc(63, 60, val)

    def set_8822_apc_wf_hb0_pa_vh_vbit(self, addrs, val):
        if self.is22:
            self.REG_HIGH = addrs[1]
            self.wapc(59, 57, val)

    def set_8822_apc_wf_hb0_padrv_vl_iv_it_bit(self, addrs, val):
        if self.is22:
            self.REG_HIGH = addrs[1]
            self.wapc(41, 36, val)
    ...
    def set_8822_apc_wf_hb0_tmx_vlo_vbit(self, addrs, val):
        if self.is22:
            self.REG_LOW = addrs[0]
            self.wapc(20, 18, val)

    # def set_8822_apc_wf_hb0_tmx_vlo_vbit(self, addr, val):
    #     if self.is22:
    #         self.REG_LOW = addr
    #         self.wapc(20, 18, val)


    # def fix_8822_apc_ant1_tx_gain_reg(self, val):
    #     self.REG_LOW = '403441a8'
    #     self.write_reg_pos(6, 6, 0b1)    # tx_gain_dr
    #     self.write_reg_pos(5, 0, val)   #
    #     self.write_reg_pos(7, 7, 0b1)   # tx_gain_dr_pulse
    ...
    def set_wf_pa_v13_sel_avddrf(self, val):
        self.REG_LOW = '40344030'
        self.write_reg_pos(29, 29, val)

    def set_wf_pa_hb_hd3_cbit(self, val):
        self.REG_LOW = '40344044'
        self.write_reg_pos(31, 29, val)

    def set_wf_pa_lb_hd2_input_cbit(self, val):
        self.REG_LOW = '40344048'
        self.write_reg_pos(24, 22, val)

    def set_wf_pa_trxsw_hd2_cbit(self, val):
        self.REG_LOW = '40344048'
        self.write_reg_pos(11, 9, val)
    ...
    def set_wf_dtmx_lb_proc_offset_cbit(self, val):
        self.REG_LOW = '4034404c'
        self.write_reg_pos(11, 9, val)

    def set_wf_dtmx_hb_proc_offset_cbit(self, val):
        self.REG_LOW = '4034404c'
        self.write_reg_pos(14, 12, val)


    def set_apc_wf_hb0_padrv_cbit(self, addr, val):
        if self.is22:
            self.REG_LOW = addr
            self.wapc(31, 27, val)

    def set_apc_wf_hb0_padrv_gbit(self, addr, val):
        if self.is22:
            self.REG_LOW = addr
            self.wapc(26, 23, val)

    def set_apc_wf_dtmx_cbit_lb(self, addr, val):
        self.REG_HIGH = addr
        self.wapc(63, 59, val)

    def set_apc_wf_dtmx_cbit_hb(self, addr, val):
        self.REG_HIGH = addr
        self.wapc(61, 57, val)

    def set_apc_wf_pa_hb_input_cbit(self, addr, val):
        self.REG_HIGH = addr
        self.wapc(52, 49, val)

    def set_apc_wf_dtmx_dac_vl_it_bit(self, addr, val):
        if self.is22:
            self.REG_LOW = addr[0]
            self.wapc(4, 0, val)
        else:
    ...
    def set_apc_wf_dtmx_dac_vl_ib_bit(self, addr, val):
        if self.is22:
            self.REG_LOW = addr[0]
            self.wapc(9, 5, val)
        else:
    ...
    # def set_22apc_wf_dac0_vl_it_bit(self, addr_low, val):
    #     self.REG_LOW = addr_low
    #     self.wapc(4, 0, val)
    #
    # def set_22apc_wf_dac0_vl_ib_bit(self, addr_low, val):
    #     self.REG_LOW = addr_low
    #     self.wapc(9, 5, val)
