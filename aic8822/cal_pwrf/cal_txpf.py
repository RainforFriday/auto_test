...
class cal_txpf:
    ...
    def fix_lb_ofdm_apc(self, ANT = "0"):
        if ANT == "0":
            self.UARTc.write_reg_mask("403441a4", "5:4", 0)
            self.UARTc.write_reg_mask("403441a4", "6", 1)
            self.UARTc.write_reg_mask("403441a4", "7", 1)
        elif ANT == "1":
            self.UARTc.write_reg_mask("403441a8", "5:4", 0)
            self.UARTc.write_reg_mask("403441a8", "6", 1)
            self.UARTc.write_reg_mask("403441a8", "7", 1)
        else:
            print("ANT INPUT ERROR: {}".format(ANT))

    def fix_lb_11b_apc(self, ANT = "0"):
        if ANT == "0":
            self.UARTc.write_reg_mask("403441a4", "5:4", 1)
            self.UARTc.write_reg_mask("403441a4", "6", 1)
            self.UARTc.write_reg_mask("403441a4", "7", 1)
        elif ANT == "1":
            self.UARTc.write_reg_mask("403441a8", "5:4", 1)
            self.UARTc.write_reg_mask("403441a8", "6", 1)
            self.UARTc.write_reg_mask("403441a8", "7", 1)
        else:
            print("ANT INPUT ERROR: {}".format(ANT))

    def release_lb_apc(self, ANT="0"):
        if ANT == "0":
            self.UARTc.write_reg_mask("403441a4", "6", 0)
            self.UARTc.write_reg_mask("403441a4", "7", 1)
        elif ANT == "1":
            self.UARTc.write_reg_mask("403441a8", "6", 0)
            self.UARTc.write_reg_mask("403441a8", "7", 1)
        else:
            print("ANT INPUT ERROR: {}".format(ANT))
    ...
    def cal_pwrf_lb_11b(self, ant_sel="0", ch_gainmap_dict = {1: "974", 7: "974", 13: "977"}):
        ...
        time.sleep(2)
        self.UARTc.sendcmd("pwrmm 1")
        self.UARTc.sendcmd("setpwr 18")
        self.UARTc.sendcmd("setrate 0 3")
        self.UARTc.sendcmd("setbw 0 0")
        self.UARTc.sendcmd("settx 1")
        self.UARTc.sendcmd("settx 0")
        ...
        for chx in ch_gainmap_dict.keys():
            ...
            self.UARTc.sendcmd(tone_on_cmd)
            time.sleep(1)
            self.fix_lb_11b_apc(ant_sel)
            time.sleep(1)

            print("REG 403422C8 : {}".format(self.UARTc.read_reg("403422c8")))
