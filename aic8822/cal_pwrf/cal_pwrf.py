...
def pa_pwrsense_off(ch=1, ANT="0"):
    if (ch<20) and (ANT=="0"):
        pa_pwrsense_lb0_off()
    elif (ch<20) and (ANT=="1"):
        pa_pwrsense_lb1_off()
...
def pa_gain_dr_release(ch = 1, ANT = "0"):
    if (ch < 20) and (ANT == "0"):
...
def cal_pwrf_lb(ant_sel="0", ch_gainmap_dict = {1: "966", 7: "96F", 13: "975"}):
    ...
    for chx in ch_gainmap_dict.keys():
        ...
        UARTc.sendcmd("tone_on {} 4 {} {}".format(tone_ant_sel, "0", ana_index_hex))
        time.sleep(1)
        pa_gain_dr_off(chx, ant_sel)
        pwrofst_msadc = MSADCX.ms_portdc()
        UARTc.sendcmd("tone_off 11")
        print("MSADC PWR SENSE REF: {:.2f}".format(pwrofst_msadc))
        pa_gain_dr_release(chx, ant_sel)
    ...
    return [l_ch, l_offset]


def cal_pwrf_hb(ant_sel="0", ch_gainmap_dict = {42: "99A", 58: "998", 106: "994", 122: "996", 138: "996", 155: "996"}, CH42_CH58_OFFSET_dB=-1.5):
    ...
    for pwr in l_msadc_pwr_42_58:
        offset1 = round((pwr_median_42_58 - pwr)*2.0)
        pwr_calx = pwr + 0.5*offset1 
