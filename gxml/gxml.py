...
class gXmlReg:
    ...
    def init(self, vlstdict_bit):
        ...
        for vdict_bit in vlstdict_bit:
            ...
            mpos = vdict_bit['pos']
            maccess = vdict_bit['access']
            if 'comment' not in vdict_bit:
                vdict_bit['comment'] = '0'

            mcomm = vdict_bit['comment']
...
class gXmlParser:
    ...
    def parse_root_xml(self, vfile): # 获取xml文件根节点信息以及 instance数据
        # archive
        # -> include
        # -> instance
        ...
        for x in mroot:
            if 'include' in x.tag:
                assert 'file' in x.attrib.keys() # attrib 判断字段是否存在
                mfile = x.attrib['file']
        ...
        # print(f'mods_file is :{self.mods_file}')
        # print(f'mods_name is :{self.mods_name}')
    ...
    def find_file(self, vname): # 找到文件位置,返回所以子文件路径
    ...
    def collect(self): # 收集，所有寄存器的字节的信息
    ...
    def filter(self, vprtn):
        ...
        try:
            for kstr in self.strs:
                if re.search(vprtn, kstr[0], re.I):  # 正则表达式
                    mlst.append(kstr)
...
def gen_bits_table(xml_file, bits_table_file):
    # path = r'./xml/aic8822/aic8822_hard.xml'
    path = xml_file
    mxml = gXmlParser(path)
    mxml.load()
    reg_list = ["reg_name,bit_name,address,val,msb,lsb,n,pos_end,pos_start,access,comment\n"]
    for modx in mxml.mods.keys():
        regs_byaddrs = mxml.mods[modx].regs_byaddr
        for regx in regs_byaddrs.keys():
            reg_address = hex(regx)
            regs_bits = regs_byaddrs[regx].bits
            regs_name = regs_byaddrs[regx].name
            # print(regs_byaddrs[regx])
            for bitx in regs_bits.keys():
                bitx_dict = regs_bits[bitx]
                # bitx_comment = " ".join(bitx_dict["comment"].replace("\n", " ").split(","))
                bitx_comment = ""
                pos_end = bitx_dict["pos"].strip().split(":")[0]
                pos_start = bitx_dict["pos"].strip().split(":")[-1]
                reg_list.append("{},{},{},{},{},{},{},{},{},{},{}\n".format(regs_name,bitx_dict["name"], str(reg_address),
                                                                    bitx_dict["val"], bitx_dict["msb"],
                                                                    bitx_dict["lsb"], bitx_dict["n"],
                                                                    pos_end, pos_start, bitx_dict["access"],
                                                                    bitx_comment))
                # print(bitx_dict)
    with open(bits_table_file, "w") as AICCSV:
        AICCSV.writelines(reg_list)
