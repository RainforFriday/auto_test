...
class FSQ(GenericInstrument):
    ...
    def set_line(self,name):
        self.write("CALC:LIM5:NAME \"{}\"".format(name))
        self.write("CALC:LIM5:UPP:STAT ON")
        self.write("CALC:LIM5::UPP:STAT ON")
        #self.write("CALC:LIM4:LOW:STAT ON")
        return True
    def set_line_fpwr(self,F,PWR):
        if len(F)==len(PWR):
            A=[]
            B=[]
            for i in range(0,len(F),1):
                A.append(str(F[i]*1e6))
                B.append(str(PWR[i]))
            #print(",".join(A))
            #print(",".join(B))
            self.write("CALC:LIM5:CONT:DATA "+",".join(A))
            self.write("CALC:LIM5:UPP:DATA "+",".join(B))
            return True
        else:
            print("freq and pwr len is not equal")
    ...
    def set_sweep_time(self, t=1000): # t unit is ms
        self.write("SWE:TIME "+str(t)+"ms")
        pass
    def get_auto_enpwr(self):
        self.set_reflvl(0)
        time.sleep(2)
        flag=1
        pwr = self.get_peak_mark()[1]
        if int(float(pwr))<30:
            self.set_reflvl(int(float(pwr)) + 20)
            time.sleep(1)
        return True
...
if __name__ == "__main__":
    ...
    FSQ1.set_Det()
    FSQ1.set_line(20)
    FSQ1.set_line_fpwr([1000,6000,6000,7000],[ -40, -40, -30, -40])
    FSQ1.set_maxhold()
    FSQ1.set_clear()
    """
