...
def find_closest_it_bit(CMPX, apc_reg, addr, val, is11b=0, with_step=0):
    ...
    if abs(val_def - val) < 0.05:  # 防止起步
        return default, default, val_def
...
if __name__ == "__main__":
    ...
    data_dict = {}
    boardno = "6e"
    bin_file = "testmode_8822_928_6e.bin"
    ...
    for index in range(0, 15):
        ...
        for setpwr in range(1, 21):  # pwr 1 ~ 24  饱和23.9,
            ...
            ib_res = find_closest_ib_bit(CMPX, apc_reg, addr_low, setpwr, with_step=0)  # 2nd 只细调
            apc_reg.REG_LOW = addr_low
            it_bit = int(apc_reg.rapc(4, 0), 2)  # int(aa, 2)
            finish_flag = 0
            if abs(ib_res[2] - setpwr) < 0.05:  # 防止起步
                finish_flag = 1
            for _ in range(8):
            ...
            data_dict["bin_version"] = bin_file
