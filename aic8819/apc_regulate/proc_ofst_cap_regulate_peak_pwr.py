...
if __name__ == "__main__":
    ...
    # for index in range(15, 18, 3):
        ...
        if index < 2:
            apc_reg.set_apc_wf_dtmx_cbit_lb(addr_high, cbit_list[index])
        else:
            apc_reg.set_apc_wf_dtmx_cbit_hb(addr_high, cbit_list[index])

        for bit in range(8):
        # for bit in [0, 1, 2] + list(range(5, 11, 3)):
        # for bit in range(0, 17, 4):
            if index < 2:
                apc_reg.set_wf_dtmx_lb_proc_offset_cbit(bit)
            else:
                apc_reg.set_wf_dtmx_hb_proc_offset_cbit(bit)
            mk_res = pxa.get_quick_peak_search(wait=2)
            # print(f'pwr: {mk_res[1]}   freq: {mk_res[0]}')
            data_dict["ofst_cbit"] = bit
            data_dict["pwrRes"] = mk_res[1]
            data_dict["frq"] = mk_res[0]
