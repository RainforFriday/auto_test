...
def find_closest_it_bit(driver, apc_reg, addr, val, is11b=0, with_step=0):
    ...
    if with_step:
        if default > 30:
            apc_reg.set_apc_wf_dtmx_dac_vl_it_bit(addr, default - 15)
            val_def_sub = get_pwr_avg(driver)
            delta = (val_def - val_def_sub) / 15
        else:
            apc_reg.set_apc_wf_dtmx_dac_vl_it_bit(addr, default + 5)
            val_def_add = get_pwr_avg(driver)
            delta = (val_def_add - val_def) / 5
...
if __name__ == "__main__":
    ...
    for index in range(3, 20):
        ...
        if rate == 'HR':
            UARTc.sendcmd('setlen 8000')
            UARTc.sendcmd('setrate 5 11')
            UARTc.sendcmd('settx 1')
            rate_table = apc_reg.get_rate_table()
        elif rate == 'MR':
            UARTc.sendcmd('setlen 4000')
            UARTc.sendcmd('setrate 5 5')
            UARTc.sendcmd('settx 1')
            rate_table = set_MR_table(UARTc, apc_reg)
            assert rate_table == 0, f"rate_table != 0, {rate_table}"
        # elif rate == 'LR':
        else:
            UARTc.sendcmd('setlen 1000')
            UARTc.sendcmd('setrate 5 0')
            UARTc.sendcmd('settx 1')
            rate_table = apc_reg.get_rate_table()
        ...
        for setpwr in range(21, 25):  # pwr 1 ~ 24  饱和23.9,
            ...
            # get
            it_res0 = find_closest_it_bit(driver, apc_reg, addr_high, setpwr, with_step=pwr_enable(setpwr, pwr_limit=pwr_limit))
            ib_res0 = find_closest_ib_bit(driver, apc_reg, addr_high, setpwr, with_step=pwr_enable(setpwr, pwr_limit=pwr_limit))
            it_init = it_res0[0]
            ib_init = ib_res0[0]
            it_bit = it_res0[1]
            finish_flag = 0
            ...
            ib_res = find_closest_ib_bit(driver, apc_reg, addr_high, setpwr, with_step=pwr_enable(setpwr, pwr_limit=pwr_limit))  # 2nd 只细调
            ...
            for _ in range(15):
                if (ib_res[1] == 0) & (finish_flag == 0):
                    ...
                    it_bit, finish_flag = boundary_protect_with_flag(it_bit, 63)
                    apc_reg.set_apc_wf_dtmx_dac_vl_it_bit(addr_high, it_bit)
                    ib_res = find_closest_ib_bit(driver, apc_reg, addr_high, setpwr, with_step=pwr_enable(setpwr, pwr_limit=pwr_limit))
                    if finish_flag:
                        break
