...
def find_closest_increment(msadc0, apc_reg, cal_ipa_vofst, res0, val, default, delta):
    '''
    ...
    apc_reg.set_wf_pa_vl_calbit(default)
    cur_default = (volt_ms_robust_avg(msadc0, 3) - cal_ipa_vofst) / res0
    print(f'cur_default: {cur_default}')

    mid = 32  # 中间点
    apc_reg.set_wf_pa_vl_calbit(mid)
    cur_mid = (volt_ms_robust_avg(msadc0,3) - cal_ipa_vofst) / res0
    print(f'cur_mid: {cur_mid}')

    # apc_reg.set_wf_pa_vl_calbit(mid - 5)
    # cur_mid_sub5 = (volt_ms_robust_avg(msadc0, 3) - cal_ipa_vofst) / res0
    # delta = (cur_mid - cur_mid_sub5) / 5

    if val >= cur_default:
        start = math.floor((val - cur_default) / delta) + default
        start = min(start, n-1)
    else:
        start = default - math.floor((cur_default - val) / delta)
        start = max(start, 0)



    # apc_reg.set_wf_pa_vl_calbit(0)
    # cur_min = (volt_ms_robust_avg(msadc0,3) - cal_ipa_vofst) / res0
    # print(f'cur_min: {cur_min}')
    #
    # apc_reg.set_wf_pa_vl_calbit(63)
    # cur_max = (volt_ms_robust_avg(msadc0,3) - cal_ipa_vofst) / res0
    ...
    apc_reg.set_wf_pa_vl_calbit(start)
    cur_start = (volt_ms_robust_avg(msadc0, 3) - cal_ipa_vofst) / res0
    ...
    if val >= cur_start:
        ...
        for i in range(start-2, n - 1):     # 增大搜索时，起步-2
            if temp != 0:
                cur_k = temp
            else:
                apc_reg.set_wf_pa_vl_calbit(i)
                cur_k = (volt_ms_robust_avg(msadc0, 3) - cal_ipa_vofst) / res0
            print(f'cur_{i}:  {cur_k}')
            apc_reg.set_wf_pa_vl_calbit(i + 1)
            cur_k1 = (volt_ms_robust_avg(msadc0, 3) - cal_ipa_vofst) / res0
            temp = cur_k1
            if (cur_k - val) * (cur_k1 - val) <= 0:
                ...
                return closest_k - default, closest_cur
            elif i == n - 2:
                print("cal done! Use maximum value")
                return (n - 1) - default, cur_k1  # 返回 31
    else:
        # 向左搜索（从mid-1到0）
        # for i in range(start - 1, -1, -1):
        for i in range(start + 1, -1, -1):  # 减小搜索时，起步+2
            if temp != 0:
                cur_k1 = temp
            else:
                apc_reg.set_wf_pa_vl_calbit(i + 1)
...
def cal_ipa_hb():
    ...
    # res_hb = 0.445
    res_hb = 0.54
    itgt = 80  # goal current / mA
    default_reg_val = 40
    experience_step_curr = 2.14
    ...
    incre, cur_final = find_closest_increment(msadc0, apc_reg, cal_ipa_vofst, res_hb, itgt, default_reg_val, experience_step_curr)
    print(f'档位增量：{incre} 电流：{cur_final}')
...
def cal_ipa_lb():
    UARTc = uart_open(8)
    res_lb = 1.12
    itgt = 88  # goal current / mA
    default_reg_val = 32
    experience_step_curr = 1.5
    ...
    # cal_ipa_v = volt_ms_robust_avg(3)
    # cal_ipa_init = (cal_ipa_v - cal_ipa_vofst) / res0
    incre, cur_final = find_closest_increment(msadc0, apc_reg, cal_ipa_vofst, res_lb, itgt, default_reg_val, experience_step_curr)
    print(f'档位增量：{incre} 电流：{cur_final}')
...
if __name__ == "__main__":
    ...
    incre, cur_final = cal_ipa_hb()
    print(incre, cur_final)
    # cal_ipa_lb()
