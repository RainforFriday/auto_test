...
def volt_ms_start(enable_filt=1, case=TestCase.TxHB.value, block=Block.DTMX.value):
    """
...
def volt_ms_core(db_line, reg_map, data_dict, pbar):
...
if __name__ == "__main__":
    ...
    xlsx_path = r'.\Table\msadc_TABLE_tx.xlsx'

    current_time = datetime.datetime.now().strftime("%m%d_%H%M%S")
    file_path = r'.\data\msadc_data_' + current_time + '.xlsx'

    volt_ms_start(enable_filt=1, case=TestCase.TxHB.value, block=Block.DTMX.value)
