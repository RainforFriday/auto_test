...
class CMP180(GenericInstrument):
    ...
    def wlan_set_wlan_Adjust_lvl(self, lvlRangInterval=0.01): # update cmp180
        ...
        # self.write("CONFigure:WLAN:MEAS{}:RFSettings:LRINterval {}".format(self.MeasNum, lvlRangInterval))
        self.write("CONFigure:WLAN:MEAS{}:RFSettings:LRSTart".format(self.MeasNum))
        time.sleep(0.2)
        return True
    ...
    def wlan_meas_tsmask_avg(self):
        ...
        try:
            ready = self.read().strip()
            time.sleep(0.3)
            # meas_pwr = self.query("FETCh:WLAN:MEAS{}:MEValuation:TSMask:AVERage?".format(self.MeasNum))
