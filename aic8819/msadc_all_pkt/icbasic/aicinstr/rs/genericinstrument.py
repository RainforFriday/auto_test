...
import os
import numpy
from aicintf.connection import ConnectionMethod
from aicintf.tcp import TcpBus
from aicintf.visa import VisaBus
...
class GenericInstrument(object):
    ...
    def open_visa(self, connection_method=ConnectionMethod.gpib, address="1"):
        self.bus = VisaBus()
        self.bus.open(connection_method, address)

    def open_tcp(self, ip_address, socket=5025):
    ...
    def write(self, buffer: object) -> object:
        self.bus.write(buffer)
        # self.write(buffer)
        self.bytes_transferred = len(buffer)
        self._print_write(buffer)

    def query(self, buffer: object) -> object:
        self.write(buffer)
        return self.read()
