from dc_monitor.aic8820h.msadc import *
...
def wf_tx_on():
    UARTc.write_reg_mask("40344088", "22:19", 14)


def wf_tx_off():
    UARTc.write_reg_mask("40344088", "22:19", 0)


def wf_rx_on():
    UARTc.write_reg_mask("40344088", "22:19", 11)
...
def measure_dc_by_channel(db_nets_by_channel):
    for db_net in db_nets_by_channel:
        cmd_str = ""
        if db_net.enable_reg_name in aic8820h_bits_dict.keys()  and db_net.bits_reg_name in aic8820h_bits_dict.keys():
            ...
            # 2 test enable
            enable_bit_reg_address = aic8820h_bits_dict[db_net.enable_reg_name].address
            enable_bit_pos = aic8820h_bits_dict[db_net.enable_reg_name].pos
            ...
            # 4 test bit
            test_bit_reg_addrss = aic8820h_bits_dict[db_net.bits_reg_name].address
            test_bit_pos = aic8820h_bits_dict[db_net.bits_reg_name].pos
...
if __name__ == "__main__":
    ...
    spec_table_path = "./MsTables/aic8820h_spec_testability_20240821X.csv"
    aic8820h_spec = SpecTable(spec_table_path)
    bits_table_path = "./MsTables/aic8820_bits_table.csv"
    aic8820h_bits = BitsTable(bits_table_path)
    aic8820h_bits_dict = aic8820h_bits.bits_dict()
    ...
    time.sleep(8)
    measure_dc_by_channel(aic8820h_spec.db_nets_by_band("WF_LB"))
    """
    ...
    UARTc.sendcmd("settx 0")
    time.sleep(8)
    measure_dc_by_channel(aic8820h_spec.db_nets_by_band(["WF_HB"]))

    UARTc.close()

    with open("./MsDatas/aic8820h_test_hb_pwr_20240812.csv", "a+") as CSVFILE:
